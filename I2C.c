...
void Process_Data(void){
	...
	if(accX<0){
		...
		if(accX>1){
			...
			send_accx[2] = '.';
			//ITOA
			send_accx[4] = ((int)accX%10) + '0';
			accX /= 10;
			send_accx[3] = ((int)accX%10) + '0';
			send_accx[1] = ((int)accX/10) + '0';
		}else{
			accX = accX*100;
			accX = round((int)accX);
			send_accx[0] = '-';
		...
		if(accX>1){
			...
			send_accx[3] = ((int)accX%10) + '0';
			accX /= 10;
			send_accx[2] = ((int)accX%10) + '0';
			send_accx[0] = (accX/10) + '0';
		}else{
			accX = accX*100;
			accX = round(accX);
			send_accx[0] = '0';
	...
	if(accY<0){
		...
		if(accY>1){
			...
			send_accy[2] = '.';
			//ITOA
			send_accy[4] = ((int)accY%10) + '0';
			accY /= 10;
			send_accy[3] = ((int)accY%10) + '0';
			send_accy[1] = ((int)accY/10) + '0';
		}else{
			accY = accY*100;
			accY = round((int)accY);
			send_accy[0] = '-';
		...
		if(accY>1){
			...
			send_accy[3] = ((int)accY%10) + '0';
			accY /= 10;
			send_accy[2] = ((int)accY%10) + '0';
			send_accy[0] = (accY/10) + '0';
		}else{
			accY = accY*100;
			accY = round(accY);
			send_accy[0] = '0';
	...
	if(accZ<0){
		...
		if(accZ>1){
			...
			send_accz[2] = '.';
			//ITOA
			send_accz[4] = ((int)accZ%10) + '0';
			accZ /= 10;
			send_accz[3] = ((int)accZ%10) + '0';
			send_accz[1] = ((int)accZ/10) + '0';
		}else{
			accZ = accZ*100;
			accZ = round((int)accZ);
			send_accz[0] = '-';
		...
		if(accZ>1){
			...
			send_accz[3] = ((int)accZ%10) + '0';
			accZ /= 10;
			send_accz[2] = ((int)accZ%10) + '0';
			send_accz[0] = (accZ/10) + '0';
		}else{
			accZ = accZ*100;
			accZ = round(accZ);
			send_accz[0] = '0';
	...
	temp = round(temp);
	send_temp[4] = 'C';
	send_temp[3] = ((int)temp%10) + '0';
	temp/=10;
	send_temp[2] = '.';
	send_temp[1] = ((int)temp%10) + '0';
	send_temp[0] = (temp/10) + '0';
}
